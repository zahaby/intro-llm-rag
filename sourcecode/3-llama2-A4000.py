...
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig, pipeline, AutoConfig, TextStreamer
...
template = """You are an assistant for question-answering tasks. Use the following pieces of retrieved context to answer the question. If you don't know the answer, just say that you don'tknow. Use six sentences three and keep the answer concise.
Question: {question}
Context: {context}
Answer:"""
...
model = LlamaForCausalLM.from_pretrained("./model",quantization_config=config,device_map='auto')
...
#        print("---append query/text to history: %s seconds ---" % (time.time() - start_time))
        start_time = time.time()

#        llm_rag_result = result['answer']
        llm_rag_result = result['result']
...
bot_interface.title = 'Chat with Llama2 7B'
